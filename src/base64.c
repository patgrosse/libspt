/*
 * Based on RIOT OS https://github.com/RIOT-OS/RIOT/blob/master/sys/base64/base64.c
 */
/*
...
static char getsymbol(unsigned char code) {
    ...
    if (code <= BASE64_SMALL_UPPER_BOUND) {
        return (char) (code + ('z' - BASE64_SMALL_UPPER_BOUND));
    }

    if (code <= BASE64_NUMBER_UPPER_BOUND) {
        return (char) (code + ('9' - BASE64_NUMBER_UPPER_BOUND));
    }
...
int8_t base64_encode(unsigned char *data_in, size_t data_in_size, \
    ...
    for (int i = 0; i < (int) (data_in_size); ++i) {
        ...
        if (njump == 4) {
            nNum = (unsigned char) (nLst << (8 - 2 * njump));
            njump = 0;
            nLst = 0;
            --i;
        } else {
        ...
        base64_out[iterate_base64_buffer++] = (unsigned char) getsymbol(nNum);
    }
    ...
    *base64_out_size = (size_t) iterate_base64_buffer;
...
int8_t base64_decode(unsigned char *base64_in, size_t base64_in_size, \
    ...
    size_t required_size = base64_required_size_from_base64(base64_in_size);
    ...
    for (int i = 1; i < (int) (base64_in_size); i++) {
        ...
        nNum = (unsigned char) (nLst + ((code & (0xFF & nm)) >> (2 * mask)));
    ...
    *data_out_size = (size_t) iterate_data_buffer;
    return BASE64_SUCCESS;
}
